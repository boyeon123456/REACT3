...
export default function Board() {
  ...
  useEffect(() => {
    ...
    const q = query(collection(db, 'posts'), orderBy('created_at', 'desc'), limit(visibleLimit));
            ...
            <Link to={`/post/${post.id}`} key={post.id} className="board-card" style={{ animationDelay: `${idx * 0.05}s` }}>
              ...
              <div className="card-bottom">
                <div className="card-author-area">
                  <div className="mini-avatar" style={{ backgroundColor: tagColors[post.board] || '#ccc', color: '#fff' }}>{(post.author || '익명')[0]}</div>
